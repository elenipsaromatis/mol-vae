#!/bin/bash
#PBS -N mol_bo_noise20_1500_pareto_expert
#PBS -l walltime=16:00:00
#PBS -l select=1:ncpus=4:mem=16gb
#PBS -j oe

# Mirrors bo_noise20_pLD50.sh but --n-iterations 1500, 20% relative LD50 noise.
# Strategy: pareto_expert
#
# FRESH run (not a resume) into a NEW out-dir -- the existing
# bo/results_pLD50/noise_20pct/ (1000 iterations, already analyzed under
...
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22+33 hpc/bo_noise20_1500_pLD50_pareto_expert.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=44+55+66 hpc/bo_noise20_1500_pLD50_pareto_expert.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=77+88+99 hpc/bo_noise20_1500_pLD50_pareto_expert.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=110 hpc/bo_noise20_1500_pLD50_pareto_expert.sh
set -euo pipefail

: "${CHECKPOINT:?Set CHECKPOINT via: qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22+33 hpc/bo_noise20_1500_pLD50_pareto_expert.sh}"
: "${SEEDS:?Set SEEDS via: qsub -v CHECKPOINT=...,SEEDS=11+22+33 hpc/bo_noise20_1500_pLD50_pareto_expert.sh (batches: 11+22+33, 44+55+66, 77+88+99, 110)}"
SEEDS="${SEEDS//+/ }"
...
python bo/run_bo.py \
  ...
  --out-dir bo/results_pLD50/noise_20pct_1500 \
  --n-iterations 1500 \
  --objective minimize \
  --seeds $SEEDS \
  --selection pareto_expert \
  --noise-scale 0.2 \
  --experiment-name bo-pLD50-1500-noise20-pareto_expert
