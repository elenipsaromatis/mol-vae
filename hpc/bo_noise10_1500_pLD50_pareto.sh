#!/bin/bash
#PBS -N mol_bo_noise10_1500_pareto
...
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22+33 hpc/bo_noise10_1500_pLD50_pareto.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=44+55+66 hpc/bo_noise10_1500_pLD50_pareto.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=77+88+99 hpc/bo_noise10_1500_pLD50_pareto.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=110 hpc/bo_noise10_1500_pLD50_pareto.sh
set -euo pipefail

: "${CHECKPOINT:?Set CHECKPOINT via: qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22+33 hpc/bo_noise10_1500_pLD50_pareto.sh}"
: "${SEEDS:?Set SEEDS via: qsub -v CHECKPOINT=...,SEEDS=11+22+33 hpc/bo_noise10_1500_pLD50_pareto.sh (batches: 11+22+33, 44+55+66, 77+88+99, 110)}"
SEEDS="${SEEDS//+/ }"
...
python bo/run_bo.py \
  ...
  --selection pareto \
  --noise-scale 0.1 \
  --experiment-name bo-pLD50-1500-noise10-pareto
