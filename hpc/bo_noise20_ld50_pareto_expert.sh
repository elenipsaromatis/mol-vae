#!/bin/bash
#PBS -N mol_bo_noise20_pareto_expert
#PBS -l walltime=08:00:00
#PBS -l select=1:ncpus=4:mem=16gb
#PBS -j oe

# Adds pareto_expert to bo/results_ld50/noise_20pct: 1000 iterations, 20%
# relative LD50 noise, all 10 RANDOM_SEEDS, overlap pool. Writes into the
# SAME out-dir as hpc/bo_noise20_ld50.sh, under seed_*/pareto_expert/ --
...
#
# --noise-scale MUST match the original run exactly (0.2) -- it's part of
# what the shared initial_indices/pca depend on being reproduced from.
#
# Submit with the SAME checkpoint used for noise_20pct:
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth hpc/bo_noise20_ld50_pareto_expert.sh

set -euo pipefail

: "${CHECKPOINT:?Set CHECKPOINT via: qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth hpc/bo_noise20_ld50_pareto_expert.sh}"
