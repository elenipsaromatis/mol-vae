#!/bin/bash
#PBS -N mol_bo_fullpool_1500_ucb
...
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22 hpc/bo_fullpool_1500_pLD50_ucb.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=33+44 hpc/bo_fullpool_1500_pLD50_ucb.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=55+66 hpc/bo_fullpool_1500_pLD50_ucb.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=77+88 hpc/bo_fullpool_1500_pLD50_ucb.sh
#   qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=99+110 hpc/bo_fullpool_1500_pLD50_ucb.sh
set -euo pipefail

: "${CHECKPOINT:?Set CHECKPOINT via: qsub -v CHECKPOINT=checkpoints/vae_solubility_XXXXXXXX_best.pth,SEEDS=11+22 hpc/bo_fullpool_1500_pLD50_ucb.sh}"
: "${SEEDS:?Set SEEDS via: qsub -v CHECKPOINT=...,SEEDS=11+22 hpc/bo_fullpool_1500_pLD50_ucb.sh (batches: 11+22, 33+44, 55+66, 77+88, 99+110)}"
SEEDS="${SEEDS//+/ }"
...
python bo/run_bo.py \
  ...
  --run-random \
  --pool full-ld50 \
  --experiment-name bo-pLD50-1500-fullpool-ucb
